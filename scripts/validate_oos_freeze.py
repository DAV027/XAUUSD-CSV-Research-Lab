...
FROZEN_EXPERIMENT_FIELDS = (
    "experiment_id", "fingerprint", "allocation_bucket", "family", "strategy_name",
    "canonical_parameters_json", "direction_mode", "stop_atr", "exit_type", "target_r",
    "time_exit_minutes", "atr_trail", "commission_round_trip_per_lot",
    "slippage_points_per_fill", "strategy_seed", "family_seed", "sampler_version",
)
...
def validate_oos_freeze(config_path: str | Path, catalog_path: str | Path, feature_path: str | Path) -> dict[str, int]:
    ...
    if actual_catalog_sha != expected_catalog_sha:
        raise ValueError(f"catalog SHA256 mismatch: expected {expected_catalog_sha}, got {actual_catalog_sha}")
    actual_feature_sha = _sha256(feature_path)
    if actual_feature_sha != expected_feature_sha:
        raise ValueError(f"feature SHA256 mismatch: expected {expected_feature_sha}, got {actual_feature_sha}")
    ...
    for candidate in candidates:
        ...
        for field in FROZEN_EXPERIMENT_FIELDS:
            ...
            if actual != expected:
                raise ValueError(f"candidate drift for {experiment_id} field {field}: expected {expected!r}, got {actual!r}")
    return {"candidate_count": len(candidates), "primary_count": primary_count, "diagnostic_count": diagnostic_count}
