...
from xau_lab.validation.edge_c_v2_activation import (
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the deterministic structural activation gate for Edge C v2"
    )
    parser.add_argument(
        "--catalog",
        type=Path,
...
def _validate_catalog_identity(catalog) -> None:
    if len(catalog) != EDGE_C_V2_DEFAULT_BUDGET:
        raise ValueError(
            f"Edge C v2 activation requires exactly {EDGE_C_V2_DEFAULT_BUDGET} catalog rows"
        )
    for experiment in catalog:
        if experiment.family != EDGE_C_V2_FAMILY:
            raise ValueError("Edge C v2 catalog contains an unexpected family")
        if experiment.allocation_bucket != EDGE_C_V2_FAMILY:
            raise ValueError("Edge C v2 catalog contains an unexpected allocation bucket")
        if experiment.strategy_name != EDGE_C_V2_STRATEGY:
            raise ValueError("Edge C v2 catalog contains an unexpected strategy")
        if experiment.sampler_version != EDGE_C_V2_SAMPLER_VERSION:
            raise ValueError("Edge C v2 catalog contains an unexpected sampler version")
        if experiment.family_seed != EDGE_C_V2_DEFAULT_SEED:
            raise ValueError("Edge C v2 catalog contains an unexpected family seed")
        if experiment.direction_mode != "combined":
            raise ValueError("Edge C v2 activation requires combined direction mode")
...
def main() -> None:
    ...
    report = run_edge_c_v2_activation(
        catalog_path=args.catalog,
        feature_path=args.features,
        output_path=args.output,
    )
    status = "PASS" if report["passed"] else "FAIL"
    print(f"Edge C v2 activation {status}")
    print(
        "Activation counts: "
        f">=10={report['at_least_10_count']}/{report['sample_size']}, "
    ...
    if not report["passed"]:
        print("Full Edge C v2 campaign remains blocked; preserve this activation report.")
