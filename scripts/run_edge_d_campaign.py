...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run or resume Edge D only after the frozen activation gate passes"
    )
    parser.add_argument(
        "--catalog",
        type=Path,
    ...
    parser.add_argument("--result-root", type=Path, default=Path("edge_d/results"))
    parser.add_argument(
        "--activation-report",
        type=Path,
...
def validate_activation_report(
    ...
    try:
        ...
        raise FileNotFoundError(
            f"Edge D activation report is required before campaign execution: {report_path}"
        ) from exc
    except (OSError, json.JSONDecodeError) as exc:
        raise ValueError(f"invalid Edge D activation report: {report_path}") from exc
    if not isinstance(payload, dict):
        raise ValueError("Edge D activation report must be a JSON object")
    ...
    expected_identity = {
        ...
        "catalog_budget": EDGE_D_DEFAULT_BUDGET,
    }
    mismatches = [
        key for key, expected in expected_identity.items() if payload.get(key) != expected
    ]
    if mismatches:
        raise ValueError("Edge D activation report identity mismatch: " + ", ".join(mismatches))
    if payload.get("passed") is not True:
        raise RuntimeError("Edge D activation did not pass; full campaign is blocked")

    actual_catalog_hash = sha256_file(catalog_path)
    if payload.get("catalog_sha256") != actual_catalog_hash:
        raise ValueError("Edge D activation report catalog hash does not match current catalog")
    actual_source_hash = sha256_file(feature_path)
    if payload.get("source_data_sha256") != actual_source_hash:
        raise ValueError("Edge D activation report feature hash does not match current source data")
    return payload


def run_edge_d_campaign(
    *,
    catalog_path: Path,
    feature_path: Path,
    result_root: Path,
    ...
    if int(seed) != EDGE_D_DEFAULT_SEED:
        raise ValueError(f"Edge D campaign seed is frozen at {EDGE_D_DEFAULT_SEED}")
...
def main() -> None:
    ...
    print("Edge D campaign run completed or resumed successfully")
