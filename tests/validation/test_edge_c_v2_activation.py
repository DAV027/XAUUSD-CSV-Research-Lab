...
def _rows(*, at_least_10=205, at_least_300=26):
    rows = []
    for index in range(ACTIVATION_SAMPLE_SIZE):
        completed = 0
        if index < at_least_10:
            completed = 10
...
def test_activation_passes_exact_205_and_26_boundaries():
    decision = evaluate_activation(_rows(at_least_10=205, at_least_300=26))
    assert decision.passed is True
    assert decision.at_least_10_count == 205
    assert decision.at_least_300_count == 26
    assert decision.reasons == ()


def test_activation_fails_with_only_204_configs_at_10_trades():
    decision = evaluate_activation(_rows(at_least_10=204, at_least_300=26))
    assert decision.passed is False
    assert "configs_with_10_trades_below_205" in decision.reasons


def test_activation_fails_with_only_25_configs_at_300_trades():
    decision = evaluate_activation(_rows(at_least_10=205, at_least_300=25))
    assert decision.passed is False
    assert "configs_with_300_trades_below_26" in decision.reasons
...
def test_activation_fails_when_total_completed_trades_is_zero():
    decision = evaluate_activation(_rows(at_least_10=0, at_least_300=0))
    assert decision.passed is False
...
def test_activation_fails_closed_on_partial_or_malformed_structural_rows():
    partial = _rows()
    partial.pop()
    assert evaluate_activation(partial).passed is False
    assert "sample_size_not_256" in evaluate_activation(partial).reasons
...
def test_activation_report_payload_is_deterministic():
    decision = evaluate_activation(_rows())
    assert decision.to_dict() == evaluate_activation(_rows()).to_dict()
    assert decision.to_dict()["sample_size"] == 256
