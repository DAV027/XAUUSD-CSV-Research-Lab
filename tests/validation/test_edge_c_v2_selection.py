...
def test_edge_c_v2_accepts_inclusive_pf_and_drawdown_boundaries():
    selected = select_edge_c_v2_candidates([
        _row("EXPBOUND", profit_factor=1.10, max_drawdown_pct=5.0),
    ])
    assert [row["experiment_id"] for row in selected] == ["EXPBOUND"]


def test_edge_c_v2_requires_strictly_positive_net_and_expectancy():
    rows = [
        _row("GOOD"),
    ...
    assert [row["experiment_id"] for row in select_edge_c_v2_candidates(rows)] == ["GOOD"]


def test_edge_c_v2_trade_frequency_is_not_an_eligibility_condition():
    selected = select_edge_c_v2_candidates([
...
def test_edge_c_v2_selection_orders_by_score_then_id_and_caps_at_six():
    rows = [_row(f"EXP{i}", final_score=float(i) / 10.0) for i in range(10)]
    selected = select_edge_c_v2_candidates(rows)
    assert [row["experiment_id"] for row in selected] == [
        "EXP9", "EXP8", "EXP7", "EXP6", "EXP5", "EXP4"
    ]

    tied = select_edge_c_v2_candidates([
...
def test_edge_c_v2_rejects_missing_nonfinite_or_malformed_metrics_and_allows_zero():
    rows = [
    ...
    assert select_edge_c_v2_candidates(rows) == []
