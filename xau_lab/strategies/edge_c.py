...
@njit(cache=True)
def _compression_breakout_retest_kernel(
    open_: np.ndarray,
    high: np.ndarray,
    low: np.ndarray,
    close: np.ndarray,
    ...
    # Monotonic rolling deques hold the strict-prior window [i-L, i).
    ...
    for i in range(n):
        ...
        if (
            ...
            if np.isfinite(width) and width >= 0.0:
                compression = width / candidate_atr <= compression_atr_ratio
        ...
        if state == ARMED:
            ...
            # If the quiet regime has ended without a decisive breakout,
            # discard the stale range. This non-compressed bar satisfies the
            # leave-before-rearm requirement; only a later fresh compression can arm.
            if not compression:
                state = SEEK
                rearm_ready = True
...
def compression_breakout_retest(ctx: StrategyContext, params: dict) -> np.ndarray:
    return _compression_breakout_retest_kernel(
        ctx.open,
        ctx.high,
        ctx.low,
        ctx.close,
...
__all__ = ["compression_breakout_retest"]
